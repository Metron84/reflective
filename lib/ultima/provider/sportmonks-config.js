...
export function statTypeIds() {
  return {
    ...
    minutes: Number(process.env.SPORTMONKS_STAT_MINUTES ?? 119),
    appearances: Number(process.env.SPORTMONKS_STAT_APPEARANCES ?? 321),
    yellow: Number(process.env.SPORTMONKS_STAT_YELLOW ?? 84),
    red: Number(process.env.SPORTMONKS_STAT_RED ?? 83),
    cleanSheet: Number(process.env.SPORTMONKS_STAT_CLEAN_SHEET ?? 194),
  };
}
