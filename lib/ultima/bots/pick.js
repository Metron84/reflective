...
export function chooseBotPick({
  ...
  for (const qid of queuePlayerIds) {
    const queued = pool.find((p) => p.id === qid);
    if (queued) return { player: queued, forced: forced.includes(queued.league), rationale: pickRationale(persona) };
  }
  ...
  const player = pool[0] ?? availablePlayers[0];
  ...
  return {
    ...
    rationale: pickRationale(persona),
  };
}
