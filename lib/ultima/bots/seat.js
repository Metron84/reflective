 ...
 */
export async function seatBots(competitionId, seatCount = ULTIMA_MAX_SEATS) {
  const db = getUltimaDb();
  if (!db) return { ok: false, error: "no_db" };

  const cap = Math.min(ULTIMA_MAX_SEATS, Math.max(2, Number(seatCount) || ULTIMA_MAX_SEATS));
