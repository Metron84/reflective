...
/** Live desk tokens: ENG ✓3 / ESP NEED 1 */
export function deskFloorTokens(counts, deficits) {
  return ULTIMA_LEAGUES.map((league) => {
    const short = ULTIMA_LEAGUE_SHORT[league] ?? league;
    const need = deficits[league] ?? 0;
    const count = counts[league] ?? 0;
    return {
      league,
      short,
      need,
      count,
      label: need > 0 ? `${short} NEED ${need}` : `${short} ✓${count}`,
    };
  });
}

export function forcedCopy(forced, labels = {}) {
  if (!forced?.length) return "";
  const names = forced.map((id) => labels[id] ?? id);
  if (names.length === 1) return `Forced. ${names[0]} minimum.`;
  return `Forced. ${names.join(" and ")} minimum.`;
}
