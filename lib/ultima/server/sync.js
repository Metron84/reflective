...
function fixtureUpsertRow(fix, league, gameweekId) {
 ...
 */
export async function syncFixturesForWindow({ from, to, gameweekId = null } = {}) {
...
export async function syncStandings() {
  ...
  for (const league of ULTIMA_LEAGUES) {
    try {
      const result = await provider.fetchStandings(league);
      for (const standing of result.rows ?? []) {
      ...
      errors.push(`${league}: ${err?.message ?? "standings failed"}`);
    }
  }
...
export async function runEuropeSync(competitionId, gameweek) {
  const db = getUltimaDb();
  if (!db) return { ok: false, error: "no_db" };

  const now = new Date();
  const from = addDays(now, -12).toISOString();
  const to = addDays(now, 8).toISOString();
  ...
  try {
    ...
    const needBackfill = !syncRow?.stats_backfilled;
    const stats = await syncFinishedMatchStats({ backfill: needBackfill });
    ...
    await writeEuropeSync(db, {
      last_ok_at: fixtures.ok ? new Date().toISOString() : syncRow?.last_ok_at ?? null,
      last_error: error,
