...
export async function expirePracticeTurn(code) {
  ...
  const opts = practiceOpts(room.code);
  // Bots pick now. Do not wait for the clock; that timer is for humans.
  await processBotTurns(room.competition_id, opts);
  ...
  if (
    ...
    return autoPickOnExpiry(room.competition_id, opts);
  }
