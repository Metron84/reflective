...
export async function listPracticeLobby(code) {
  ...
  const seated = [...humans, ...bots];
  const seats = Array.from({ length: ULTIMA_MAX_SEATS }, (_, index) => {
    const seat = seated[index];
    if (!seat) return { team_name: "Bot", bot: true };
    return {
      team_name: seat.is_bot ? "Bot" : seat.team_name || "Bot",
      bot: Boolean(seat.is_bot),
    };
  });
