...
export async function createPracticeRoom({ userId, seasonManager, solo = true }) {
  ...
  if (solo) {
    const started = await startDraft(competition.id, userId, practiceOpts(code));
    if (!started.ok) return { ok: false, code: "UNAVAILABLE", message: started.error };
  }
...
export async function expirePracticeTurn(code) {
  ...
  if (
    ctx?.state?.state === "live" &&
    ctx.state.turn_expires_at &&
    new Date(ctx.state.turn_expires_at).getTime() <= Date.now()
  ) {
    return autoPickOnExpiry(room.competition_id, practiceOpts(room.code));
  }
