...
function lineFor(event, teamName, payload, playerName) {
...
export async function getCompetitionNews(competitionId, limit = 24) {
  ...
  const { data: events, error } = await db
    ...
    .in("event", ["pick_made", "market_add", "xi_saved", "trade_executed", "trade_proposed"])
...
async function formatRows(rows, nested) {
  ...
  const playerIds = [
    ...new Set(
      rows.flatMap((r) => {
        const p = r.payload ?? {};
        return [p.player_id, p.add].filter(Boolean);
      }),
    ),
  ];
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      event: row.event,
      at: row.created_at,
      line: lineFor(row.event, teamName, payload, playerName),
    };
  });
}
