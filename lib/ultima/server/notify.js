...
function notifyKey(kind) {
  if (String(kind ?? "").startsWith("draft_reminder")) return "draft_reminder";
  return kind;
}

async function prefsAllow(managerId, kind) {
  const db = getUltimaDb();
  if (!db || !managerId) return true;
  ...
  const prefs = data.notify_prefs && typeof data.notify_prefs === "object" ? data.notify_prefs : {};
  return prefs[notifyKey(kind)] !== false;
}
...
export async function notifyManagerOnce({
  ...
  if (!(await prefsAllow(managerId, kind))) {
