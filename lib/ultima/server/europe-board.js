...
function emptyDesk(extra = {}) {
  return {
    ...
    lastOkAt: extra.lastOkAt ?? null,
    ...extra,
  };
}

export function europeFormIncomplete(desk) {
  const seen = new Set((desk?.form?.teams?.all ?? []).map((row) => row.league));
  return ULTIMA_LEAGUES.some((league) => !seen.has(league));
}

export function shouldRefreshEuropeForm(desk) {
  if (!desk || desk.emptyReason === "sync") return true;
  return europeFormIncomplete(desk);
}
...
export async function getEuropeDesk(competitionId) {
  ...
  const { data: fixturesRaw, error: fixtureErr } = await db
  ...
  const [standingsRows, rosters, finished, stats, history] = await Promise.all([
    loadPage(
      db,
      "ultima_standings",
    ...
    loadPage(
      ...
      (q) => q.not("player_id", "is", null).order("updated_at", { ascending: false }).limit(1500),
    ),
    loadPage(
      db,
      "ultima_standings_history",
      "as_of, league, club_id, position",
      (q) => q.order("as_of", { ascending: false }).limit(800),
    ),
  ]);
  ...
  const playerForm = buildPlayerForm(playerStatRows).map((row) => {
    ...
    return {
      ...
      name: sample?.name,
      club: sample?.club,
      leagueCode: ULTIMA_LEAGUE_SHORT[row.league] ?? row.league,
  ...
  const gwLive =
    ...
    gameweek?.window_end &&
    Date.now() >= new Date(gameweek.window_start).getTime() &&
    Date.now() <= new Date(gameweek.window_end).getTime();

  return {
    gameweek: gwLive && Number.isInteger(gameweek?.number) ? gameweek.number : null,
    fixtures: strip,
    emptyReason,
    ...
    ratingsAvailable,
    lastOkAt: syncRow?.last_ok_at ?? null,
  };
}

/** Hub paint must not wait on Sportmonks. Client / cron fill the desk. */
export function kickEuropeSync(competitionId) {
  if (!competitionId) return;
  Promise.resolve()
    .then(() => ensureEuropeDesk(competitionId))
    .catch((err) => {
      console.error("ultima europe kick", err?.message ?? err);
    });
}

/** First seated-manager open fills upcoming fixtures if cron has not run yet. */
export async function ensureEuropeDesk(competitionId) {
  ...
  let desk = await getEuropeDesk(competitionId);
  const needsForm = europeFormIncomplete(desk);
  if (desk.fixtures?.length && !needsForm) return desk;
  ...
  const lastOk = syncRow?.last_ok_at ? new Date(syncRow.last_ok_at).getTime() : 0;
  const stale = !lastOk || Date.now() - lastOk > 15 * 60 * 1000;
  if (!stale) return desk;
