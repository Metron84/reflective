...
import { managerOnClock, buildSnakeDraftOrder, picksUntilManager } from "@/lib/ultima/draft/snake";
import {
  countByLeague,
  wouldBreakFloor,
  remainingSlots,
...
export async function executePick({
  ...
  const nextOn =
    nextPickPreview <= TOTAL_PICKS ? managerOnClock(ctx.order, nextPickPreview) : null;
  ...
  if (nextPick <= TOTAL_PICKS && !opts.skipBotChain) {
