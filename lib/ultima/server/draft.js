...
const BOT_PICK_TAKEN_RETRIES = 5;
const botChainLocks = new Map();
...
function isAutoSeat(manager) {
  return Boolean(manager && (manager.is_bot || manager.auto_draft));
}
...
export async function loadDraftContext(competitionId, { includeAvailable = true } = {}) {
  ...
  const stored = state.draft_order;
  const order =
    Array.isArray(stored) && stored.length >= 2 && stored.length <= ULTIMA_MAX_SEATS
      ? stored
      : (managers ?? []).map((m) => m.id);
...
export async function buildDraftRoomPayload(ctx, { manager, queue = [], extra = {} }) {
  ...
  const slotsLeft = remainingSlots(myPickCount);
  const counter = floorCounterState(myCounts, slotsLeft);
  const forced = forcedLeagues(myCounts, slotsLeft);
  const youPickIn = picksUntilManager(ctx.order, ctx.state.current_pick, manager.id);

  const othersNeed = [];
  for (const seat of ctx.managers) {
    if (seat.id === manager.id) continue;
    const seatPicks = ctx.picks
      .filter((p) => p.manager_id === seat.id)
      .map((p) => p.ultima_players)
      .filter(Boolean);
    const deficits = leagueDeficits(countByLeague(seatPicks));
    const short = ULTIMA_LEAGUES.find((l) => deficits[l] > 0);
    if (!short) continue;
    othersNeed.push({
  ...
  return {
    ...
    on_clock: onClockManager
      ? {
          ...
          is_bot: Boolean(onClockManager.is_bot),
          auto_seat: isAutoSeat(onClockManager),
    ...
    floor: {
      ...
      deficits: counter.deficits,
      slotsLeft: counter.slotsLeft,
      mode: counter.mode,
      forced,
    },
    others_need: othersNeed,
    you_pick_in: youPickIn,
    turn_expires_at: ctx.state.turn_expires_at ?? null,
    queue,
...
async function claimPickStepwise({
  ...
  clearUndraftedCache(competitionId);
...
export async function executePick({
  ...
  const liveAvailable = await getUndraftedPlayers(competitionId, { fresh: true });
  ...
  if (!validation.ok) {
    if (autoPicked && validation.code === "NOT_YOUR_TURN") {
      return validation;
    }
    if (!autoPicked) {
      return validation;
  ...
  const seats = draftSeatCount(ctx.order);
  const budget = draftPickBudget(ctx.order);
  const round = Math.ceil(pickNumber / seats);
  const forcedFlag = forced || Boolean(validation.forced);
  const forcedLeagueValue = forcedLeague ?? validation.forcedLeague ?? null;
  const nextPickPreview = pickNumber + 1;
  const nextOn =
    nextPickPreview <= budget ? managerOnClock(ctx.order, nextPickPreview) : null;
  const nextMgr = nextOn
    ? ctx.managers.find((seat) => seat.id === nextOn.managerId)
    : null;
  const timerSeconds = isAutoSeat(nextMgr) ? 1 : ctx.timerSeconds;
  ...
  if (nextPick <= budget && !opts.skipBotChain) {
  ...
  return {
    ...
    pickNumber,
    nextPick,
    complete: Boolean(claim.complete) || nextPick > budget,
  };
}

export async function processBotTurns(competitionId, options = {}) {
  if (botChainLocks.get(competitionId)) {
    return { ok: true, picked: 0, skipped: true, locked: true };
  }
  botChainLocks.set(competitionId, true);
  ...
  try {
    while (safety > 0) {
      ...
      for (let i = 0; i < BOT_PICK_TAKEN_RETRIES; i += 1) {
        let candidates;
        try {
        ...
        const pick = candidates.find((row) => !triedIds.has(row.player.id));
        if (!pick) {
          if (!candidates.length && i === 0) {
            return pickFail(
              "BOT_PICK_FAILED",
        ...
        triedIds.add(pick.player.id);
        lastPlayerId = pick.player.id;
        ...
        if (result.code === "NOT_YOUR_TURN") {
          return { ok: true, picked, skipped: true };
        }

        if (result.code === "PICK_TAKEN") {
          console.warn("Ultima PICK_TAKEN on bot pick — retrying from live undrafted set", {
            competition_id: competitionId,
      ...
      if (!result?.ok) {
        return {
          ...
          message:
            result?.message ??
            `Bot exhausted PICK_TAKEN retries. player=${lastPlayerId ?? "none"} pick=${pickNumber}`,
          picked,
    ...
    botChainLocks.delete(competitionId);
  }
}
...
async function computeBotCandidates(ctx, manager) {
  ...
  const availablePlayers = await getUndraftedPlayers(ctx.competitionId, { fresh: true });
  ...
  return listBotPickCandidates({
    ...
    draftRound: Math.ceil(ctx.state.current_pick / draftSeatCount(ctx.order)),
  });
}
...
export async function startDraft(competitionId, actorId, options = {}) {
  ...
  const seatCount = Math.min(
    ULTIMA_MAX_SEATS,
    Math.max(2, Number(opts.seatCount) || ULTIMA_MAX_SEATS),
  );
  await seatBots(competitionId, seatCount);
  ...
  if ((allManagers ?? []).length < seatCount) {
    return { ok: false, error: "not_enough_seats" };
  }

  const seated = [...(allManagers ?? [])]
    .sort((a, b) => (Number(a.draft_slot) || 99) - (Number(b.draft_slot) || 99))
    .slice(0, seatCount);
  const order = opts.keepOrder
    ? seated.map((m) => m.id)
    : shuffle(seated.map((m) => m.id));
...
export async function advanceDraft(competitionId, options = {}) {
  const opts = mergeDraftOpts({ maxBotPicks: 20, ...options });
  ...
  const botTurn = isAutoSeat(manager);
  const expired =
  ...
  if (botTurn) {
    ...
    const clockMoved =
      after.current_pick != null && after.current_pick !== clock.state.current_pick;

    if (botResult?.locked) {
      return { ok: true, skipped: true, ...after };
    }
    if (!botResult?.ok) {
      if (clockMoved || botResult.code === "NOT_YOUR_TURN") {
        return { ok: true, advanced: clockMoved, skipped: !clockMoved, ...after };
      }
      return {
        ok: false,
        advanced: false,
    ...
    if ((botResult.picked ?? 0) === 0 && after.on_clock_is_auto && !botResult.skipped) {
      return {
        ok: false,
        advanced: false,
    ...
    return { ok: true, advanced: clockMoved || (botResult.picked ?? 0) > 0, ...after };
  }

  if (expired) {
    if (opts.skipHumanExpiry && !manager?.auto_draft) {
      return { ok: true, skipped: true, ...(await clockAfterAdvance(competitionId)) };
    }
...
async function clockAfterAdvance(competitionId) {
  ...
  return {
    ...
    on_clock_is_bot: Boolean(nextMgr?.is_bot),
    on_clock_is_auto: isAutoSeat(nextMgr),
  };
}
...
async function notifyOnClockIfHuman(competitionId, timerSeconds) {
  ...
  const manager = ctx.managers.find((m) => m.id === ctx.onClock?.managerId);
  if (!manager || isAutoSeat(manager)) return;
