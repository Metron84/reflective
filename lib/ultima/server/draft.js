...
export async function processBotTurns(competitionId, options = {}) {
  ...
  while (safety > 0) {
    ...
    let pick;
    try {
      pick = await computeBotPick(ctx, manager);
    } catch (err) {
      ...
      return {
        ...
        message: err?.message ?? "Bot could not pick.",
        picked,
      };
    }
    if (!pick?.player) {
      return {
        ok: false,
        code: "BOT_PICK_FAILED",
        message: "Bot found no valid player.",
        picked,
      };
    }
    ...
    if (!result.ok) {
      return {
        ...
        message: result.message ?? "Bot pick did not land.",
        picked,
      };
    }
    picked += 1;
  ...
  if (botTurn) {
    ...
    if (!botResult?.ok) {
      return {
        ...
        message: botResult?.message ?? "Bot could not pick.",
        ...after,
      };
    }
    if ((botResult.picked ?? 0) === 0 && after.on_clock_is_bot) {
      return {
        ...
        advanced: false,
        code: "BOT_PICK_FAILED",
        message: "Bot did not advance the clock.",
        ...after,
      };
    }
