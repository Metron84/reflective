...
export function getCompareRow(mode, personId) {
  ...
  if (mode === "classic") return person.canonical;
  return person.rows.find((r) => r.category === mode) ?? null;
}

/** All persons guessable in a mode (one entry per person). */
export function getPersonsInMode(mode) {
  const { persons } = buildRegistry();
  const out = [];
  for (const person of persons.values()) {
    if (mode === "classic" || person.rows.some((r) => r.category === mode)) {
      out.push(person);
    }
  }
  return out;
...
/** Classic daily answer pool: canonical row must have league data. */
export function getAnswerPersons(mode) {
  if (mode === "classic") {
    return getPersonsInMode("classic").filter((p) => p.canonical?.league != null);
  }
...
export function getGuessSuggestions(mode) {
  return getPersonsInMode(mode).map((p) => ({
    personId: p.personId,
    name: p.name,
    context: contextLine(p.canonical),
  }));
}
...
function findPersonsByInput(mode, input) {
  ...
  for (const row of loadRows()) {
    if (mode !== "classic" && row.category !== mode) continue;
...
export function resolveSubmission(mode, { guess, personId }) {
  ...
  return {
    ...
    options: persons.map((person) => ({
      person,
      row: getCompareRow(mode, person.personId) ?? person.canonical,
    })),
  };
}
