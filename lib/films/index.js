...
import { FILMS_TAB_FORMATS } from "@/lib/config";
...
/** Long-form films for the Films tab. */
export function getFilmsTabFilms() {
  return getAllFilms().filter(
    (film) => film.format && FILMS_TAB_FORMATS.includes(film.format)
  );
}

/** Short-form films for the Shorts tab. */
export function getShortsFilms() {
  return getAllFilms().filter((film) => film.format === "short_film");
}

/** @deprecated Use getFilmsTabFilms(). */
export function getGridFilms() {
  return getFilmsTabFilms();
}
