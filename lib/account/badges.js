...
const BADGE_DEFS = [
  ...
  },
  {
    slug: "week_of_wins",
    check: ({ modeStats }) => {
      const classic = modeStats.find((m) => m.slug === "classic");
      return (classic?.bestStreak ?? 0) >= 7;
    },
    progress: ({ modeStats }) => {
      const classic = modeStats.find((m) => m.slug === "classic");
      const current = classic?.currentStreak ?? classic?.bestStreak ?? 0;
      if (current >= 7) return null;
      return {
        label: "Seven-day Classic win streak.",
        current,
        target: 7,
      };
    },
  },
  {
    ...
    check: ({ modeStats }) => {
      const classic = modeStats.find((m) => m.slug === "classic");
      return (classic?.bestStreak ?? 0) >= 30;
    },
    progress: ({ modeStats }) => {
      const classic = modeStats.find((m) => m.slug === "classic");
      const current = classic?.bestStreak ?? 0;
      if (current >= 30) return null;
      return {
        label: "Thirty-day Classic win streak.",
        current,
        target: 30,
      };
    },
