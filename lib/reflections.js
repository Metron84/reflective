...
const SEED_FILES = {
  ...
  "best-message": "content/reflections/best-message-nominees.json",
  "best-interview": "content/reflections/best-interview-nominees.json",
  "best-soundbite": "content/reflections/best-soundbite-nominees.json",
};
...
export async function getNomineesByCategory() {
  ...
  if (supabase) {
    const { data, error } = await supabase
      .from("nominees")
      .select(
        "id, category, title, youtube_id, clip_url, context_line, sort, clip_start_seconds, nation"
      )
...
/**
 * Authenticated tally for the given category slugs only.
 * Callers must only request categories the member has already voted in.
 */
export async function getStandingsForCategories(categorySlugs) {
  ...
  for (const slug of Object.keys(byCategory)) {
    byCategory[slug].sort((a, b) => b.votes - a.votes);
  }
  return byCategory;
}
