...
export function getGames() {
  ...
  return list
    ...
    .map((game) => {
      ...
      if (game.slug === "the-crest") {
        if (!CREST_ENABLED) return null;
        return {
          ...game,
          status: "live",
          href: game.href ?? CREST_PLAY_URL,
          cover: game.cover ?? "/crest/crest-card.png",
        };
      }
      if (game.slug === "king-of-the-burgers") {
        if (!KOTB_ENABLED) return null;
        return {
          ...game,
          status: "live",
        };
